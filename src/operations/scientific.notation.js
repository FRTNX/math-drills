...
var scientific_notation = function (operation, difficulty) { return __awaiter(void 0, void 0, void 0, function () {
    var difficultyProfile, terms, formattedTerms, numberOfTerms, i, _a, max, min, multiplicand, exponent, questionLatex, correctAnswer, operator, options, result, question, existingQuestion, error_1;
        ...
        switch (_b.label) {
            case 0:
                ...
                for (i = 0; i < numberOfTerms; i++) {
                    _a = __read(difficultyProfile.multiplicandParams, 2), max = _a[0], min = _a[1];
                    multiplicand = Number(Math.floor(Math.random() * (max - min) + min) / 100).toFixed(1);
                    exponent = random.apply(void 0, __spreadArray([], __read(difficultyProfile.exponents), false));
                    terms.push(Number(multiplicand) * Math.pow(10, exponent));
                ...
                if (terms.length > 1) {
                    ...
                    options = { division: { styles: ['fraction'] } };
                    result = generateQuestionLatex(operator, terms, formattedTerms, options);
