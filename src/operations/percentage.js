...
var percentage = function (operation, difficulty) { return __awaiter(void 0, void 0, void 0, function () {
    var difficultyProfile, variables, valueToHide, questionLatex, correctAnswer, base, percentage_1, rate, rate, percentage_2, base, rate, base, percentage_3, question, existingQuestion, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                difficultyProfile = DIFFICULTY_PROFILES[difficulty];
                variables = ['rate', 'base', 'percentage'];
                valueToHide = variables[random(0, variables.length)];
                if (valueToHide === 'rate') {
                    base = random.apply(void 0, __spreadArray([], __read(difficultyProfile.baseRange), false));
                    percentage_1 = random.apply(void 0, __spreadArray([], __read(difficultyProfile.percentageRange), false));
                    rate = (percentage_1 / base) * 100;
                    questionLatex = "?% of " + base + " = " + percentage_1;
                    correctAnswer = rate;
                }
                if (valueToHide === 'base') {
                    rate = random.apply(void 0, __spreadArray([], __read(difficultyProfile.rateRange), false));
                    percentage_2 = random.apply(void 0, __spreadArray([], __read(difficultyProfile.percentageRange), false));
                    base = percentage_2 / (rate / 100);
                    questionLatex = rate + "% of ? = " + percentage_2;
                    correctAnswer = base;
                }
                if (valueToHide === 'percentage') {
                    rate = random.apply(void 0, __spreadArray([], __read(difficultyProfile.rateRange), false));
                    base = random.apply(void 0, __spreadArray([], __read(difficultyProfile.baseRange), false));
                    percentage_3 = (rate / 100) * base;
                    questionLatex = rate + "% of " + base + " = ?";
                    correctAnswer = percentage_3;
                }
