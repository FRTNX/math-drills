...
const binomials = async (operation: string, difficulty: number): Promise<IQuestion> => {
    ...
    const formattedBinomial = binomialTerms.map((factorArray) => factorArray.join('')).join(` ${operator} `);
