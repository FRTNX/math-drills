...
var summation = function (operation, difficulty) { return __awaiter(void 0, void 0, void 0, function () {
    var difficultyProfile, sum, lowerLimit, sequenceLength, upperLimit, sequence, operator, variables, variable, rightSide, addend_1, exponent_1, questionLatex, question, existingQuestion, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                difficultyProfile = DIFFICULTY_PROFILES[difficulty];
                sum = 0;
                lowerLimit = random.apply(void 0, __spreadArray([], __read(difficultyProfile.lowerLimitRange), false));
                sequenceLength = random.apply(void 0, __spreadArray([], __read(difficultyProfile.sequenceRange), false));
                upperLimit = lowerLimit + sequenceLength;
                sequence = Array.from({ length: upperLimit - (lowerLimit - 1) }, function (_, i) { return (lowerLimit - 1) + 1 + i; });
                operator = difficultyProfile.operators[random(0, difficultyProfile.operators.length)];
                variables = ['n', 'i'];
                variable = variables[random(0, variables.length)];
                if (operator == 'addition') {
                    addend_1 = random(1, 5);
                    rightSide = variable + " + " + addend_1;
                    sequence.map(function (n) { return sum += n + addend_1; });
                }
                ;
                if (operator == 'exponent') {
                    exponent_1 = random(2, 5);
                    rightSide = variable + "^" + exponent_1;
                    sequence.map(function (n) { return sum += Math.pow(n, exponent_1); });
                }
                questionLatex = "\\displaystyle\\sum_{" + variable + "=" + lowerLimit + "}^{" + upperLimit + "} " + rightSide;
